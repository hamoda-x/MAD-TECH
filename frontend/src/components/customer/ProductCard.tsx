...
function generateWhatsAppUrl(product: Product, price: number, lang: string, whatsappNumber: string, customerDetails?: string): string {
  const message = lang === "ar"
    ? encodeURIComponent(
        `مرحباً! أنا مهتم بطلب:\n\n*${product.name}*\nالسعر: $${price.toFixed(2)}\n\n${customerDetails || "يرجى تقديم مزيد من التفاصيل."}`
      )
    : encodeURIComponent(
        `Hi! I'm interested in ordering:\n\n*${product.name}*\nPrice: $${price.toFixed(2)}\n\n${customerDetails || "Please provide more details."}`
      );
      ...
      >
        ...
        <div className="flex flex-1 flex-col p-4">
          ...
          <div className="mt-auto pt-4 space-y-2">
            <button
              className="w-full flex items-center justify-center gap-2 rounded-xl bg-mad-dark-hover border border-mad-border px-4 py-2.5 text-sm font-medium text-mad-text-primary transition-all hover:bg-mad-cyan/10 hover:border-mad-cyan/30 hover:text-mad-cyan"
              onClick={(e) => {
                e.stopPropagation();
                onClick?.();
              }}
            ...
            <button
              className="w-full whatsapp-btn flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 font-semibold text-white transition-all hover:scale-[1.02]"
              onClick={(e) => {
                e.stopPropagation();
            ...
            >
              ...
              <span>{lang === "ar" ? "اطلب عبر واتساب" : "Order via WhatsApp"}</span>
            </button>
          </div>
        </div>
      </article>
